...
# This identifier existed in an earlier demo path and represents synthetic
# hardware, not a real worker process. Runtime Authority must never report it
...
def _worker_snapshot(db: sqlite3.Connection) -> dict[str, Any]:
    ...
    for row in rows:
        worker_id = str(row[0])
        if worker_id in SYNTHETIC_WORKER_IDS:
            continue
    ...
    return {
        "registered": len(workers), "online": counts.get("ONLINE", 0),
...
def _memory_snapshot(db: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        "backend": "sqlite-wal", "status": "ONLINE" if integrity == "OK" else "DEGRADED",
...
def _scheduler_snapshot(db: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        "status": "READY", "queueDepth": counts.get("QUEUED", 0) + counts.get("PENDING_WORKER", 0),
        "running": counts.get("RUNNING", 0), "completed": counts.get("COMPLETED", 0),
        "failed": counts.get("FAILED", 0), "byStatus": counts, "authority": "tasks_table",
    }
...
def _provider_snapshot(provider_router: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "samples": metrics.get("samples", 0), "successes": metrics.get("successes", 0),
        "failures": metrics.get("failures", 0), "cooldowns": metrics.get("cooldowns", {}),
    }


def snapshot(store: Any, provider_router: Any, started_at: float) -> dict[str, Any]:
    """Return only observable runtime state; synthetic demo hardware is excluded."""
    db = store.db
    ...
    return {
        "type": "system_diagnostic", "source": "runtime_authority", "observedAt": _now(),
        "runtime": {"status": "ONLINE", "service": "sarembok-ve-cloud-runtime", "port": int(os.getenv("SAREMBOK_PORT", "9000")), "domain": os.getenv("SAREMBOK_DOMAIN", "sarembok.com"), "uptimeSeconds": int(time.time() - started_at)},
        "workers": workers, "agents": agents, "compute": _compute_snapshot(workers, provider_router),
        "memory": memory, "scheduler": scheduler, "provider": provider,
...
def render_markdown(data: dict[str, Any]) -> str:
    ...
    runtime, workers, agents = data["runtime"], data["workers"], data["agents"]
    compute, memory, scheduler, provider = data["compute"], data["memory"], data["scheduler"], data["provider"]
    lines = [
        "## Sarembok Runtime Diagnostic", "", f"**Observed:** `{data['observedAt']}`", f"**Authority:** `{data['source']}`", "",
        "### Runtime", f"- Status: **{runtime['status']}**", f"- Service: `{runtime['service']}`", f"- Port: `{runtime['port']}`", f"- Domain: `{runtime['domain']}`", f"- Uptime: `{runtime['uptimeSeconds']}s`", "",
        "### Registered Workers", f"- Registered: **{workers['registered']}**", f"- Online: **{workers['online']}**", f"- Stale: **{workers['stale']}**", f"- Offline: **{workers['offline']}**",
    ]
    for worker in workers["workers"]:
