...
class ProviderRouter:
    ...
    def configured(self) -> list[ProviderSpec]:
        ...
        if openai: result['OpenAI'] = ProviderSpec('OpenAI', os.getenv('LLM_MODEL', 'gpt-5-mini'), 'openai', 'https://api.openai.com/v1/chat/completions', openai)
        router = os.getenv('OPENROUTER_API_KEY', '').strip()
        if router: result['OpenRouter'] = ProviderSpec('OpenRouter', os.getenv('OPENROUTER_MODEL', 'openai/gpt-oss-120b'), 'openai', 'https://openrouter.ai/api/v1/chat/completions', router)
        groq = os.getenv('GROQ_API_KEY', '').strip()
        if groq: result['Groq'] = ProviderSpec('Groq', os.getenv('GROQ_MODEL', 'openai/gpt-oss-120b'), 'openai', 'https://api.groq.com/openai/v1/chat/completions', groq)
        gemini = os.getenv('GEMINI_API_KEY', '').strip()
        if gemini: result['Gemini'] = ProviderSpec('Gemini', os.getenv('GEMINI_MODEL', 'gemini-3.8-flash'), 'gemini', 'https://generativelanguage.googleapis.com/v1beta/interactions', gemini)
        custom = os.getenv('LLM_ENDPOINT_URL', '').strip()
        if custom: result['Custom'] = ProviderSpec('Custom', os.getenv('LLM_MODEL', 'llama-3.1-8b'), 'openai', custom, os.getenv('LLM_API_KEY', 'dummy'))
    ...
    def _extract_gemini(payload: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        ...
        for step in payload.get('steps', []) or []:
            ...
            for block in step.get('content', []) or []:
                if block.get('type') == 'text' and block.get('text'): parts.append(block['text'])
        if not parts:
            for cand in payload.get('candidates', []) or []:
                for part in cand.get('content', {}).get('parts', []) or []:
                    if part.get('text'): parts.append(part['text'])
        if not parts: raise RuntimeError('Gemini returned no model output')
    ...
    def _sse_events(response) -> Any:
        ...
        for raw_line in response:
            ...
            if line.startswith('event:'): event_name = line[6:].strip()
                ...
                if data_lines:
                    raw_data = '\n'.join(data_lines)
                    if raw_data == '[DONE]': yield event_name or 'done', None
                    else:
                        try: yield event_name or 'message', json.loads(raw_data)
    ...
    def _request(self, spec: ProviderSpec, system_prompt: str, prompt: str, messages: list[dict[str, str]], deadline: float) -> tuple[str, dict[str, Any], str]:
        if spec.kind == 'gemini':
            if self.gemini_api == 'generatecontent':
                ...
                headers = {'Content-Type': 'application/json', 'x-goog-api-key': spec.key}; api_name = 'generateContent'
            else:
                url = spec.endpoint
                ...
                headers = {'Content-Type': 'application/json', 'x-goog-api-key': spec.key}; api_name = 'interactions'
        else:
            url = spec.endpoint
            ...
            if spec.name == 'OpenRouter': headers.update({'HTTP-Referer': 'https://sarembok.com', 'X-Title': 'Sarembok VE'})
        ...
        while True:
            ...
            if remaining <= 0: raise TimeoutError('provider deadline exceeded')
            ...
            try:
                ...
                if spec.kind == 'gemini': text, usage = self._extract_gemini(payload)
                else: text, usage = str(payload['choices'][0]['message']['content']).strip(), payload.get('usage', {}) or {}
                return text, usage, api_name
            except urllib.error.HTTPError as exc:
                if exc.code not in TRANSIENT_HTTP or attempts >= 2: raise
                delay = min(1.5, 0.35 * (2 ** (attempts - 1)))
                if deadline - time.monotonic() <= delay + 1: raise
                time.sleep(delay)
    ...
    def _request_stream(self, spec: ProviderSpec, system_prompt: str, prompt: str, messages: list[dict[str, str]], deadline: float, on_delta: Callable[[str], None]) -> tuple[str, dict[str, Any], str, float]:
        if spec.kind != 'gemini' or self.gemini_api != 'interactions': raise RuntimeError('streaming is currently supported for Gemini Interactions only')
        ...
        while True:
            ...
            remaining = deadline - time.monotonic()
            if remaining <= 0: raise TimeoutError('provider deadline exceeded')
            timeout = min(self.provider_timeout, max(1, remaining)); started = time.monotonic()
            try:
                text_parts: list[str] = []; usage: dict[str, Any] = {}; ttft_ms = None
                    ...
                    for event_name, event in self._sse_events(response):
                        ...
                        if event_name == 'step.delta' or event.get('event_type') == 'step.delta':
                            ...
                            if delta.get('type') == 'text' and delta.get('text'):
                                ...
                                if ttft_ms is None: ttft_ms = round((time.monotonic() - started) * 1000, 1)
                                text_parts.append(chunk); on_delta(chunk)
                        elif event_name == 'interaction.completed' or event.get('event_type') == 'interaction.completed':
                            interaction = event.get('interaction') or {}; usage = interaction.get('usage') or event.get('usage') or {}
                if not text_parts: raise RuntimeError('Gemini stream returned no model output')
    ...
    def generate_stream(self, system_prompt: str, prompt: str, messages: list[dict[str, str]], on_delta: Callable[[str], None]) -> ProviderResult:
        ...
        if not providers: raise RuntimeError('no language-model provider configured')
        deadline = time.monotonic() + self.total_timeout; failures: list[str] = []
        for spec in providers:
            started = time.monotonic()
            try:
                if spec.kind == 'gemini' and self.gemini_api == 'interactions': text, usage, api_name, ttft_ms = self._request_stream(spec, system_prompt, prompt, messages, deadline, on_delta)
                else:
                    text, usage, api_name = self._request(spec, system_prompt, prompt, messages, deadline); ttft_ms = round((time.monotonic() - started) * 1000, 1); on_delta(text)
                latency_ms = round((time.monotonic() - started) * 1000, 1)
                self._history.append({'provider': spec.name, 'model': spec.model, 'latency_ms': latency_ms, 'ttft_ms': ttft_ms, 'attempts': 1, 'api': api_name, 'ok': True, 'timestamp': time.time()})
                return ProviderResult(text, spec.name, spec.model, latency_ms, 1, api_name, usage, ttft_ms)
            except Exception as exc:
                latency_ms = round((time.monotonic() - started) * 1000, 1); failures.append(f'{spec.name}:{type(exc).__name__}')
    ...
    def generate(self, system_prompt: str, prompt: str, messages: list[dict[str, str]]) -> ProviderResult:
        ...
        if callback is not None: return self.generate_stream(system_prompt, prompt, messages, callback)
        providers = self.configured()
        if not providers: raise RuntimeError('no language-model provider configured')
        deadline = time.monotonic() + self.total_timeout; failures: list[str] = []
        for spec in providers:
    ...
    def metrics(self) -> dict[str, Any]:
        entries = list(self._history); ok = [e for e in entries if e.get('ok')]
