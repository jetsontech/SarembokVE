...
def main():
    with tempfile.NamedTemporaryFile(suffix=".db") as f:
        ...
        # Simulate the production V2 database contract: active_tasks already
        # exists before Scheduler V3 adds its own columns.
        assert "active_tasks" in {
            r[1] for r in db.execute("PRAGMA table_info(workers)")
        }
        ...
        claimed = rpc(
            scheduler,
            "ClaimTaskV3",
            {"taskId": first["taskId"], "workerId": "v3-a"},
        )
        ...
        rpc(
            scheduler,
            "ClaimTaskV3",
            {"taskId": retry["taskId"], "workerId": "v3-a"},
        )
        failed = rpc(
            scheduler,
            "FailTask",
        ...
        db.execute(
            "UPDATE tasks SET retry_at='1970-01-01T00:00:00+00:00' WHERE task_id=?",
            (retry["taskId"],),
        )
        db.commit()
        scheduler.reap()
        ...
        db.execute(
            "UPDATE workers SET last_heartbeat='1970-01-01T00:00:00+00:00' WHERE worker_id='v3-a'"
        )
        db.commit()
