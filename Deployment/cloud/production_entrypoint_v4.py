"""Hardened SarembokVE production boundary, release candidate v4."""
...
def token_hash(token: str) -> str:
    return hashlib.sha256((os.getenv("SAREMBOK_WORKER_TOKEN_HASH_SALT", "").strip() + token).encode()).hexdigest()
...
def worker_token_valid(worker_id: str, token: str) -> bool:
    ...
    if not row or row[1]:
        return False
    valid = hmac.compare_digest(str(row[0]), token_hash(token))
    if valid:
...
def identify(method: str, params: dict[str, Any]) -> tuple[str, str]:
    ...
    wid = str(params.get("workerId") or "").strip()
    wt = str(params.get("workerToken") or "").strip()
...
def validate(request: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    EXECUTION_ID.set(execution)
    EXECUTION_SUBJECT.set(subject)
...
def dispatch(method: str, params: dict[str, Any]) -> dict[str, Any]:
    ...
    # Never forward enrollment credentials or worker authentication material to
    # the legacy dispatcher/event layer.
    clean = {k:v for k,v in params.items() if not k.startswith("_srbk_") and k not in {"enrollmentToken","workerToken","authToken"}}
    ...
    if method == "RegisterWorker" and isinstance(result, dict):
        ...
        if worker_id:
            worker_issued = secrets.token_urlsafe(32)
            ensure_worker_credentials()
            cloud.store.db.execute(f"INSERT OR REPLACE INTO {WORKER_TOKEN_TABLE}(worker_id,token_hash,created_at,last_used_at,revoked_at) VALUES(?,?,?,?,NULL)", (worker_id,token_hash(worker_issued),cloud.now(),cloud.now()))
            cloud.store.db.commit()

    if method == "ListWorkers":
        result = annotate_workers(result)
    if not isinstance(result, dict): result = {"value": result}
    result = scrub(dict(result))

    # The newly enrolled worker needs the credential exactly once. It is not a
    # pre-existing secret and is never written to logs or the database in clear.
    if worker_issued:
        result["workerToken"] = worker_issued
...
def annotate_workers(value: Any) -> Any:
    if isinstance(value,list): return [annotate_workers(x) for x in value]
    if isinstance(value,dict):
        out={k:annotate_workers(v) for k,v in value.items()}
