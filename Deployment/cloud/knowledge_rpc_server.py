...
import asyncio, importlib.util, json, os, time, sys
import websockets
...
if spec is None or spec.loader is None: raise RuntimeError(f"Unable to load cloud server: {CLOUD_SERVER_PATH}")
cloud_server=importlib.util.module_from_spec(spec); sys.modules[spec.name]=cloud_server; spec.loader.exec_module(cloud_server)
...
def _authoritative_snapshot() -> dict:
    # Runtime truth boundary: do not manufacture or heartbeat a worker here.
...
def _is_runtime_diagnostic(prompt: str) -> bool:
    text=prompt.lower(); markers=("system diagnostic","runtime diagnostic","registered workers","compute capabilities","persistent memory status","scheduler status","provider currently serving")
    return sum(1 for marker in markers if marker in text)>=2


def dispatch(method: str, params: dict) -> dict:
    if method=="GetRuntimeInfo": return _authoritative_snapshot()
...
async def handler(websocket) -> None:
    peer=getattr(websocket,"remote_address",None); cloud_server.LOG.info("connection_open peer=%s",peer)
    try:
        async for raw in websocket:
            request=None; token_ctx=None
            try:
                if isinstance(raw,str) and len(raw.encode("utf-8"))>cloud_server.MAX_REQUEST_BYTES: raise ValueError("request_too_large")
                request=json.loads(raw); method,params=cloud_server.validate_request(request)
                ...
                first_delta_at=[None]; started_at=time.perf_counter()
                if stream_requested:
                    loop=asyncio.get_running_loop(); request_id=request.get("id")
                    def emit_delta(text: str) -> None:
                        if first_delta_at[0] is None: first_delta_at[0]=time.perf_counter()
                        event={"jsonrpc":"2.0","method":"SarembokChat.delta","params":{"id":request_id,"text":text}}
                        future=asyncio.run_coroutine_threadsafe(websocket.send(json.dumps(event,separators=(",",":"))),loop); future.result(timeout=10)
                    token_ctx=set_stream_callback(emit_delta)
                async with cloud_server.get_db_lock(): result=await asyncio.to_thread(cloud_server.dispatch,method,params)
                if token_ctx is not None:
                    reset_stream_callback(token_ctx); token_ctx=None
                    if isinstance(result,dict):
                        metadata=result.get("metadata")
                        if isinstance(metadata,dict): metadata["streamed"]=True; metadata["ttft_ms"]=round((first_delta_at[0]-started_at)*1000,1) if first_delta_at[0] is not None else None
                response={"jsonrpc":"2.0","id":request.get("id"),"result":result}
                cloud_server.LOG.info("rpc_success method=%s request_id=%s streamed=%s",method,request.get("id"),stream_requested)
            except PermissionError as exc:
                response={"jsonrpc":"2.0","id":request.get("id") if isinstance(request,dict) else None,"error":{"code":-32001,"message":str(exc)}}; cloud_server.LOG.warning("rpc_auth_failed peer=%s",peer)
            except Exception as exc:
                if token_ctx is not None:
                    try: reset_stream_callback(token_ctx)
                    except Exception: pass
                response={"jsonrpc":"2.0","id":request.get("id") if isinstance(request,dict) else None,"error":{"code":-32000,"message":str(exc)}}; cloud_server.LOG.warning("rpc_error peer=%s error=%s",peer,exc)
            await websocket.send(json.dumps(response,separators=(",",":")))
    except websockets.exceptions.ConnectionClosed: pass
    finally: cloud_server.LOG.info("connection_close peer=%s",peer)
...
async def process_http_request(connection,request):
    ...
    if not html_str: html_str="<!DOCTYPE html><html><body><h1>Sarembok VE Cloud Runtime</h1><p>Status: ONLINE</p></body></html>\n"
    if hasattr(connection,"respond"):
        resp=connection.respond(200,html_str)
        try: del resp.headers["Content-Type"]
        except Exception: pass
        resp.headers["Content-Type"]="text/html; charset=utf-8"; resp.headers["Cache-Control"]="no-cache"; return resp
...
if __name__=="__main__": asyncio.run(cloud_server.main())
