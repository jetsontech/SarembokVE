...
async def one(token, i):
    ...
    return {
        ...
        'ws_connect_ms': round(connected, 1),
        'ttft_client_ms': round(first_delta_ms, 1) if first_delta_ms is not None else None,
        'ttft_server_ms': meta.get('ttft_ms'),
        'provider_ms': meta.get('latency_ms'),
        'provider': meta.get('provider'),
        'model': meta.get('model'),
        'api': meta.get('provider_api'),
        'streamed': meta.get('streamed'),
        'response': result.get('response', '')[:100],
        'delta_chars': len(''.join(text_parts)),
    }
...
async def main():
    ...
    print(json.dumps({
        ...
        'ttft_client_p50_ms': round(statistics.median(ttfts), 1) if ttfts else None,
        'ttft_client_max_ms': round(max(ttfts), 1) if ttfts else None,
        'provider_p50_ms': round(statistics.median(providers), 1) if providers else None,
        'provider_max_ms': round(max(providers), 1) if providers else None,
    }, indent=2))
