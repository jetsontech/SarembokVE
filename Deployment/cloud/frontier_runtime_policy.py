...
def _purge_synthetic_workers(runtime: Any) -> None:
    ...
    doomed=[str(row[0]) for row in rows if str(row[0]) in SYNTHETIC_WORKER_IDS or str(row[0]).startswith(SYNTHETIC_WORKER_PREFIXES)]
    if doomed:
        runtime.store.db.executemany("DELETE FROM workers WHERE worker_id=?",[(wid,) for wid in doomed]); runtime.store.db.commit()
...
def apply(runtime: Any) -> None:
    ...
    def safe_run_terminal(cls, command: str) -> dict[str, Any]:
        ...
        except ValueError as exc:return {"error":f"invalid_command: {exc}"}
        if not argv or argv[0] not in SAFE_COMMANDS:return {"error":"command_not_allowlisted"}
        if any(token in command for token in (";","&&","||","|",">","<","`","$(","${")):return {"error":"shell_syntax_not_permitted"}
        if argv[0]=="git" and (len(argv)<2 or argv[1] not in {"status","log","diff","show","rev-parse"}):return {"error":"git_operation_not_allowlisted"}
        if argv[0] in {"python","python3"} and len(argv)>1 and argv[1] not in {"--version","-V"}:return {"error":"python_execution_disabled_in_production"}
        try:
        ...
        except subprocess.TimeoutExpired:return {"command":command,"error":"diagnostic_command_timeout"}
        except Exception as exc:return {"command":command,"error":type(exc).__name__}
