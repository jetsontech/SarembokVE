#!/usr/bin/env python3
"""SarembokVE production UI smoke test v2.

Validates the public application shell, browser session, WebSocket/JSON-RPC,
provider routing, BrowserRender, browser worker 9100, navigation, theme control,
visible Dialogue Execute, and Markdown rendering.
"""
...
UA = "SarembokVE-Production-Smoke/2.0"
...
def http_json(url: str) -> dict:
    with urllib.request.urlopen(request(url, "application/json"), timeout=15) as r:
        return json.loads(r.read().decode("utf-8"))


def http_text(url: str) -> str:
    with urllib.request.urlopen(request(url, "text/html"), timeout=15) as r:
...
def main() -> int:
    failures: list[str] = []
    print("===== SAREMBOKVE PRODUCTION UI SMOKE V2 =====")

    try:
        with urllib.request.urlopen(request(f"{BASE}/health", "text/plain"), timeout=10) as r:
    ...
    try:
        ...
        print("[PASS] public /session: token issued; scope includes core UI methods")
    except Exception as exc:
    ...
    ws_code = dedent('''
        ...
        async def main():
            async with websockets.connect("ws://127.0.0.1:9000/ws", open_timeout=8, close_timeout=5, ping_interval=20, ping_timeout=20) as ws:
                ...
                chat = await rpc("chat", "SarembokChat", {"prompt":"Use the exact text **SAREMBOK_PROVIDER_SMOKE** in your response.","stream":False,"sessionId":"production-smoke-v2"})
    ...
    playwright_code = dedent('''
        ...
        with sync_playwright() as p:
            ...
            if input_box.count() != 1 or execute.count() != 1:
                raise RuntimeError("dialogue Execute controls not found in Dialogue view")
            if not input_box.is_visible() or not execute.is_visible():
                diagnostic = page.evaluate('''() => {
                    const ids = ["directive-input", "execute-button"];
                    return ids.map(id => {
                        ...
                        for (let i=0; n && i<6; i++, n=n.parentElement) {
                            ...
                            a.push({tag:n.tagName,id:n.id,class:n.className,display:s.display,visibility:s.visibility,opacity:s.opacity,hidden:n.hidden});
                        }
    ...
    if failures:
        for failure in failures:
            print("[FAIL]", failure)
        return 1
    print("[PASS] ALL PRODUCTION UI SMOKE CHECKS V2")
    return 0
