...
class SchedulerV3:
    ...
    def ensure_schema(self) -> None:
        ...
        # V2 creates active_tasks, but V3 also migrates it so the scheduler is
        # safe against a bare/older SQLite database.
        ...
        self.db.execute("UPDATE workers SET active_tasks=0 WHERE active_tasks IS NULL OR active_tasks<0")
    ...
    def fail(self, p: dict[str, Any]) -> dict[str, Any]:
        ...
        if int(task[2] or 0) < int(task[3] or self.default_attempts):
            ...
            stamp = now()
            self.db.execute("UPDATE tasks SET status='FAILED',lease_expires_at=NULL,error=?,completed_at=?,updated_at=? WHERE task_id=?", (reason, stamp, stamp, tid))
            result = "FAILED"
    ...
    def offline(self, p: dict[str, Any]) -> dict[str, Any]:
        ...
        stamp = now()
        self.db.execute("UPDATE workers SET status='OFFLINE',last_disconnect_at=?,queued_tasks=0,lease_expires_at=NULL WHERE worker_id=?", (stamp, wid))
        queued = self.db.execute("SELECT task_id FROM tasks WHERE assigned_worker_id=? AND status='QUEUED'", (wid,)).fetchall()
        for task in queued:
            self.db.execute("UPDATE tasks SET status='PENDING_WORKER',assigned_worker_id=NULL,retry_at=NULL,updated_at=? WHERE task_id=? AND status='QUEUED'", (stamp, task[0]))
        ...
        for task in running:
            if int(task[1] or 0) < int(task[2] or self.default_attempts):
                self.db.execute("UPDATE tasks SET status='RETRY_WAIT',assigned_worker_id=NULL,retry_at=?,lease_expires_at=NULL,error=?,updated_at=? WHERE task_id=? AND status='RUNNING'", (after(self.retry_delay), "worker_offline", stamp, task[0]))
                retried += 1
            else:
                self.db.execute("UPDATE tasks SET status='FAILED',assigned_worker_id=NULL,lease_expires_at=NULL,error=?,completed_at=?,updated_at=? WHERE task_id=? AND status='RUNNING'", ("worker_offline", stamp, stamp, task[0]))
                failed += 1
            self.db.execute("UPDATE workers SET active_tasks=MAX(active_tasks-1,0) WHERE worker_id=?", (wid,))
        self.event("WORKER_OFFLINE", wid, None, releasedQueuedTasks=len(queued), retriedRunningTasks=retried, failedRunningTasks=failed)
        reassigned = self.assign_pending()
        self.db.commit()
        return {"workerId": wid, "status": "OFFLINE", "releasedQueuedTasks": len(queued), "retriedRunningTasks": retried, "failedRunningTasks": failed, "reassignedTasks": reassigned}
    ...
    def dispatch(self, method: str, params: dict[str, Any]):
        ...
        if method == "HeartbeatV3": return self.heartbeat(params)
