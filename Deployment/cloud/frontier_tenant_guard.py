...
def handle_user_rpc(runtime: Any, method: str, params: dict[str, Any], subject: str) -> tuple[bool, dict[str, Any] | None]:
    ...
    if method == "StoreMemory":
        ...
        db.execute(
            ...
            (memory_id, tier, key, value, subject, stamp),
        )
        db.commit()
    ...
    if method in {"ListMemories", "SearchMemories"}:
        query = str(params.get("query", "")).strip() if method == "SearchMemories" else ""
        tier = str(params.get("tier", "")).strip().upper()
        limit = min(100, max(1, int(params.get("limit", 50))))
        sql = "SELECT memory_id,tier,key,value,agent_id,created_at FROM memories WHERE agent_id=?"
    ...
    if method == "RecallMemory":
        ...
        row = db.execute(
            "SELECT memory_id,tier,key,value,agent_id,created_at FROM memories WHERE key=? AND agent_id=? ORDER BY created_at DESC LIMIT 1",
            (key, subject),
        ).fetchone()
    ...
    if method == "DeleteMemory":
        ...
        cur = db.execute("DELETE FROM memories WHERE memory_id=? AND agent_id=?", (memory_id, subject))
        db.commit()
    ...
    if method == "SpatialVisualRecall":
        ...
        sql = "SELECT memory_id,key,value,created_at FROM memories WHERE tier='SPATIAL' AND agent_id=?"
        args: list[Any] = [subject]
        if query:
            sql += " AND (key LIKE ? OR value LIKE ?)"
            args.extend([f"%{query}%", f"%{query}%"])
